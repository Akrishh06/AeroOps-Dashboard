...
export const ColorOrb: React.FC<OrbProps> = ({
  ...
  const dimValue = parseInt(dimension.replace("px", ""), 10) || 192;
  ...
  return (
    <div className={cn("flex w-full items-center justify-center", className)}>
      <motion.div
        ...
        data-panel
        className={cn(
          "bg-background border-border relative z-[3] flex w-full max-w-full flex-col items-center overflow-hidden border max-sm:bottom-5",
        )}
        initial={false}
        animate={{
  ...
  return (
    <footer className="mt-auto flex h-[44px] w-full items-center justify-center whitespace-nowrap select-none">
      <div className="flex w-full items-center justify-center gap-2 px-3 max-sm:h-10 max-sm:px-2">
        ...
        <Button
          type="button"
          className="flex h-fit min-w-0 flex-1 justify-end rounded-full px-2 !py-0.5"
          variant="ghost"
          onClick={triggerOpen}
        >
...
>(function InputForm({ onSubmitMessage, onAfterSend }, ref) {
        ...
        {showForm && (
          ...
          >
            ...
            <textarea
              ...
              name="message"
              className="text-foreground placeholder:text-dim h-full w-full resize-none scroll-py-2 rounded-md bg-transparent p-4 outline-0"
...
function KeyHint({ children, className }: { children: string; className?: string }) {
  return (
    <kbd
      className={cn(
        "text-foreground border-border flex h-6 w-fit items-center justify-center rounded-sm border px-[6px] font-sans",
        className,
      )}
    >
      {children}
